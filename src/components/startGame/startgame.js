...
class StartGame extends Component {
  ...
  componentDidMount() {
    if (this.props.user) {
      this.props.getCurrentGame(this.props.user.id);
    }
  }
          ...
          <Fragment>
            ...
            >
              <UploadImageForm
                ...
                imgData={this.state.imgData}
              />
              <button disabled={checkArrayLength !== 8} type="submit">
                upload{checkArrayLength}
              </button>
            </form>
            {this.props.gameImgsData.length > 0 && (
              <a
                href={` http://${ip.address()}:3000/game-custom/${
                  this.props.gameImgsData[0].user
                }`}
                target='"_blank"'
              >
                {" "}
                http://{ip.address()}:3000/game-custom/
                {this.props.gameImgsData[0].user}
              </a>
            )}
          </Fragment>
